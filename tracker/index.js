...
(window => {
  ...
  const eventClass = /^simplemetrics--([a-z]+)--([\w]+[\w-]*)$/;
  const eventSelect = "[class*='simplemetrics--']";

  const trackingDisabled = () =>
    (localStorage && localStorage.getItem('simplemetrics.disabled')) ||
    (dnt && doNotTrack()) ||
  ...
  if (!window.simplemetrics) {
    const simplemetrics = eventValue => trackEvent(eventValue);
